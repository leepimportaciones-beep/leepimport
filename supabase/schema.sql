...
create table pedidos(id_pedido bigint generated always as identity primary key,codigo_pedido varchar(30) unique,nombre_cliente varchar(150) not null,telefono_cliente varchar(80) not null,observaciones_cliente text,estado varchar(50) default 'PENDIENTE_PRESUPUESTAR',total_bruto numeric(14,2) default 0,descuento_porcentaje numeric(8,2) default 0,descuento_fijo numeric(14,2) default 0,total_descuento numeric(14,2) default 0,total_final numeric(14,2) default 0,observaciones_presupuesto text,mensaje_whatsapp_cliente text,mensaje_whatsapp_presupuesto text,fecha_presupuestado timestamptz,fecha_finalizado timestamptz,fecha_venta_concretada timestamptz,activo boolean default true,fecha_alta timestamptz default now(),fecha_modificacion timestamptz default now(),check(estado in('PENDIENTE_PRESUPUESTAR','PRESUPUESTADO','FINALIZADO','VENTA_CONCRETADA','CANCELADO')));
